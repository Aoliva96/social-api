    ...
    await User.find({})
      ...
      .then((userData) => {
        ...
        if (userData.length === 1) {
          ...
          resObj = {
            message: `Showing all ${userCount()} users`,
            users: `${userData}`,
          };
        }
        res.json(resObj);
  ...
  async getSingleUser(req, res) {
    await User.findOne({ _id: req.params.userId })
      ...
      .then((userData) => {
        ...
        res.json({
          message: `Showing user with id ${req.params.userId}`,
          user: `${userData}`,
        });
      })
      .catch((err) => {
        ...
        if (userData.thoughts.length) {
          ...
          res.json({
            message: `Deleted user with id ${
              req.params.userId
            } and ${thoughtCount(req.params.userId)} associated thought(s)`,
          });
        } else {
  ...
  async addFriend(req, res) {
    await User.findOneAndUpdate(
      ...
      .then((userData) => {
        ...
        res.json({
          message: `Added new friend with id ${
            req.params.friendId
